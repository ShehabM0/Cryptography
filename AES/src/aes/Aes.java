...
public class Aes {
    ...
    Aes(String plaintext, String key, Boolean is_hex_text) throws FileNotFoundException {
        helper.readTables();
    ...
    private ArrayList<ArrayList<Pair>> ShiftRows(ArrayList<ArrayList<Pair>> stateGrid) {
        for(int i = 0; i < state_rows; i++) {
            for(int k = 0; k < i; k++) { // shift i times
                Pair p = stateGrid.get(i).get(0);
                for(int j = 1; j < state_cols; j++)
                    stateGrid.get(i).set(j - 1, stateGrid.get(i).get(j));
                stateGrid.get(i).set(state_cols - 1, p);
            }
        }
    ...
    private ArrayList<ArrayList<Pair>> MixColumns(ArrayList<ArrayList<Pair>> stateGrid) {
        ArrayList<ArrayList<Pair>> res = new ArrayList<>();
        String[][] matmutli = helper.matmulti;
...
//                    else if(matmulti_str.equals("02")) multi_res = MixColumns02(stateGrid_p);
                    if(matmulti_str.equals("01"))
                    ...
                    multi_res = xor(multi_res, current_state);
    ...
    public String encrypt() {
        ...
        for(int i = 0; i < 10; i++) {
            addRoundKey = AddRoundKey(stateGrid, i); 
            subByte = SubByte(addRoundKey);
            shiftRows = ShiftRows(subByte);
            if(i < 9) {
                mixColumns = MixColumns(shiftRows);
                stateGrid = mixColumns;
            }
        }
        stateGrid = AddRoundKey(shiftRows, 10);
